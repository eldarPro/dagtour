...
const toTgHref = (val: string) => {
  if (val.startsWith('http')) return val;
  if (/^\+?[0-9]/.test(val)) return `https://t.me/+${val.replace(/^\+/, '')}`;
  return `https://t.me/${val.replace(/^@/, '')}`;
};

const toVkHref = (val: string) => {
  if (val.startsWith('http')) return val;
...
const SocialLinks: React.FC<Props> = ({ whatsapp, telegram, vk }) => {
  const items = [
    whatsapp && { icon: <WhatsAppIcon />, href: toWaHref(whatsapp) },
    telegram && { icon: <TelegramIcon />, href: toTgHref(telegram) },
    vk       && { icon: <VKIcon />,       href: toVkHref(vk) },
  ].filter(Boolean) as { icon: React.ReactNode; href: string }[];

  if (items.length === 0) return null;

  return (
    <div className="reviews-link-row social-links-row">
      <div className="reviews-link-left">
        <IonIcon icon={chatbubblesOutline} style={{ fontSize: 18, color: 'var(--ion-text-color)' }} />
